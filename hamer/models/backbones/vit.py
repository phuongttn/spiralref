...
def vit(cfg):
    ...
    Behavior:
      ...
      - encoder runs only on kept tokens of each image
...
class ViT(nn.Module):
    def __init__(
        ...
        print(
            ...
            "TOKEN_DROP_MODE=per_image_spiral_cos_once"
        )
    ...
    def forward_features(self, x):
        ...
        for b in range(B):
            ...
            kept = self._run_blocks(x[b:b + 1].index_select(1, keep)).squeeze(0)
            x_full[b].index_copy_(0, keep, kept)

            if drop.numel() > 0:
                x_full[b].index_copy_(
                    0,
                    drop,
                    self.last_norm(x[b:b + 1].index_select(1, drop)).squeeze(0),
                )
